...
class SearchView(FormView):
  ...
  def lookup_account_function(self, fields):
    lookup_dict = {
      'email': Q(user__email__startswith=fields.get('email')) ,
      'first_name': Q(user__first_name__istartswith=fields.get('first_name')),
      'last_name': Q(user__last_name__startswith=fields.get('last_name')),
      'skill_serial': Q(skill__data__serial=fields.get('skill_serial')),
      'project_name': Q(projectpages__name__startswith=fields.get('project_name'))
    }
